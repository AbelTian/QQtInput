...
add_sdk(QSoftKeyboard, $$add_target_name())
#add_sdk_header_no_postfix(QSoftKeyboard, $$add_target_name(), QSoftKeyboard)

#-------------------------------------------------
#用户工程配置
#-------------------------------------------------
include($${PWD}/qsoftkeyboard_header.pri)
include($${PWD}/qsoftkeyboard_source.pri)

CONFIG += continue_build
contains(CONFIG, continue_build){
    system("touch $${PWD}/qsoftkeyboard.cpp")
}

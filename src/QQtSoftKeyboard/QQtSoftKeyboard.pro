...
add_source_dir($$PWD/Soft-keyboard)
add_sdk(QQtSoftKeyboard, $$add_target_name())
#add_sdk_header_no_postfix(QQtSoftKeyboard, $$add_target_name(), QQtSoftKeyboard)

#-------------------------------------------------
#用户工程配置
#-------------------------------------------------
include($${PWD}/qqtsoftkeyboard_header.pri)
include($${PWD}/qqtsoftkeyboard_source.pri)

CONFIG += continue_build
contains(CONFIG, continue_build){
    system("touch $${PWD}/qqtsoftkeyboard.cpp")
}
